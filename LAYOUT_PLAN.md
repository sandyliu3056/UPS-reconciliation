...
確認是死碼：HTML 元素已經不在，兩處 JS 還在（`index.html:9492` 設 disabled、
`14258` 綁 onclick），兩處都用 `$("#bCsv") &&` 保護所以不會出錯。**建議刪，零風險。**

### B. 管理員分頁

不用寫新程式。`ADMIN_ONLY` 的三層判斷都已經在了（`5119` 定義、`6587` 藏按鈕、
`6666` 擋入口）；它現在會出現，是因為 `LOCAL_ACCOUNTS` 裡那個帳號的
`role` 是 `"admin"`（`5061`）。**把那一個字改成 `"user"` 分頁就消失**，
之後切 Supabase 也不受影響 —— 那時 role 是從 `app_metadata` 讀的。

### C. 住宅判定 API 有沒有在用

這題程式可以自己回答：`resi_source` 預設是 `"system"`（`3613`、`4937`），
只有等於 `"api"` 而且 `resi_api_url` 有填才會打（`9782`）。
...
實際用量：`act` 39 處、`act ghost` 100 處、`act run` 1 處（Process Invoice）、
